...
const skillGroups = [
  ...
  },
  {
    ...
    title: 'CAM / CNC',
    skills: [
      { name: 'HSMWorks', level: 85 },
      { name: 'GearTeq', level: 85 },
      { name: 'Mastercam', level: 50 },
    ],
  },
  {
    icon: Code,
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {skillGroups.map(g => (
            <div key={g.title} className="p-6 rounded-xl bg-card border border-border">
              ...
              {g.skills.map(s => (
                <SkillBar key={s.name} name={s.name} level={s.level} visible={isVisible} />
              ))}
            </div>
          ))}
        </div>
